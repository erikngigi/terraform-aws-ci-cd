...
resource "aws_cloudfront_origin_access_control" "startpage_prod_site" {
  name                              = "${var.startpage_bucket_prod_id}-oac"
  description                       = "Startpage production website origin access control"
  origin_access_control_origin_type = "s3"
  signing_behavior                  = "always"
  signing_protocol                  = "sigv4"
}

resource "aws_cloudfront_origin_access_control" "startpage_dev_site" {
  name                              = "${var.startpage_bucket_dev_id}-oac"
  description                       = "Startpage development webiste origin access control"
  origin_access_control_origin_type = "s3"
  signing_behavior                  = "always"
  signing_protocol                  = "sigv4"
}
...
resource "aws_acm_certificate" "startpage_prod_site" {
  domain_name       = "${var.startpage_prod_domain_name}.${var.domain_name}"
  validation_method = "DNS"
  key_algorithm     = "EC_prime256v1"
  ...
  tags = {
    Name = "Startpage production website certificate"
  }
}

resource "aws_acm_certificate" "startpage_dev_site" {
  domain_name       = "${var.startpage_dev_domain_name}.${var.domain_name}"
  validation_method = "DNS"
  key_algorithm     = "EC_prime256v1"
  ...
  tags = {
    Name = "Startpage development website certificate"
  }
}
...
resource "aws_cloudfront_distribution" "hugo_prod_site" {
  ...
  tags = {
    Name = "Hugo production cloudfront distribution"
  }
}
...
resource "aws_cloudfront_distribution" "hugo_dev_site" {
  ...
  tags = {
    Name = "Hugo development cloudfront distribution"
  }
}

resource "aws_cloudfront_distribution" "startpage_prod_site" {
  enabled             = true
  is_ipv6_enabled     = true
  default_root_object = "index.html"
  price_class         = "PriceClass_100"
  comment             = "Startpage production website distribution"
  http_version        = "http2and3"

  aliases = ["${var.startpage_prod_domain_name}.${var.domain_name}"]

  origin {
    domain_name              = var.startpage_bucket_prod_domain_name
    origin_id                = "S3-${var.startpage_bucket_prod_id}"
    origin_access_control_id = aws_cloudfront_origin_access_control.startpage_prod_site.id
  }

  default_cache_behavior {
    allowed_methods  = ["GET", "HEAD", "OPTIONS"]
    cached_methods   = ["GET", "HEAD"]
    target_origin_id = "S3-${var.startpage_bucket_prod_id}"
  ...
  viewer_certificate {
    acm_certificate_arn      = aws_acm_certificate.startpage_prod_site.arn
    ssl_support_method       = "sni-only"
    minimum_protocol_version = "TLSv1.2_2021"
  }

  tags = {
    Name = "Startpage production cloudfront distribution"
  }
}

resource "aws_cloudfront_distribution" "startpage_dev_site" {
  enabled             = true
  is_ipv6_enabled     = true
  default_root_object = "index.html"
  price_class         = "PriceClass_100"
  ...
  aliases = ["${var.startpage_dev_domain_name}.${var.domain_name}"]

  origin {
    domain_name              = var.startpage_bucket_dev_domain_name
    origin_id                = "S3-${var.startpage_bucket_dev_id}"
    origin_access_control_id = aws_cloudfront_origin_access_control.startpage_dev_site.id
  }

  default_cache_behavior {
    allowed_methods  = ["GET", "HEAD", "OPTIONS"]
    cached_methods   = ["GET", "HEAD"]
    target_origin_id = "S3-${var.startpage_bucket_dev_id}"
  ...
  viewer_certificate {
    acm_certificate_arn      = aws_acm_certificate.startpage_dev_site.arn
    ssl_support_method       = "sni-only"
    minimum_protocol_version = "TLSv1.2_2021"
  }

  tags = {
    Name = "Startpage development cloudfront distribution"
  }
}
